...
class ConditionalFlowMatcher:
    ...
    def sample_location_and_conditional_flow(self, x0, x1, y=None, t=None, return_noise=False, time_dist='uniform'):
        """
        ...
        if t is None:
            if time_dist == 'uniform':
                ...
                # mu, sigma = -0.4, 1.0
                # normal_samples = torch.randn(x0.shape[0]).type_as(x0) * sigma + mu
                # t = 1 / (1 + torch.exp(-normal_samples))  # Sigmoid transformation
                mu, sigma = 0.0, 1.0
                t = self.sample_logit_normal(mu, sigma, size=x0.shape[0]).type_as(x0)
...
class ExactOptimalTransportConditionalFlowMatcher(ConditionalFlowMatcher):
    ...
    def __init__(self, sigma: Union[float, int] = 0.0, dist='euclidean', power=2, method="scipy"):
        r"""Initialize the ConditionalFlowMatcher class. It requires the hyper-parameter $\sigma$.
    ...
    def sample_location_and_conditional_flow(self, x0, x1, y=None, t=None, return_noise=False):
        r"""
        Compute the sample xt (drawn from N(t * x1 + (1 - t) * x0, sigma))
        and the conditional vector field ut(x1|x0) = x1 - x0, see Eq.(15) [1]
        ...
        if y is not None:
            raise("Conditional OT-CFM is only implemented in scipy version.")
        x0, x1 = self.ot_sampler.sample_plan(x0, x1)
        return super().sample_location_and_conditional_flow(x0, x1, y, t, return_noise)
...
class MLEConditionalFlowMatcher(ConditionalFlowMatcher):
    def __init__(self, p_mean:Union[float, int] = 0.0, p_sigma:Union[float, int] = 0.0, sigma: Union[float, int] = 0.0):
        r"""Initialize the ConditionalFlowMatcher class. It requires the hyper-parameter $\sigma$.

        Parameters
        ----------
        p_mean : Union[float, int] Mean of target distribution
        p_sigma : Union[float, int] sigma of target distribution
        sigma : Union[float, int]
        """
        self.p_mean = p_mean
